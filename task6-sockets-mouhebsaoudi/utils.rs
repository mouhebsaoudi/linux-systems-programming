...
/// Open a socket to the specified hostname and port.
pub fn connect_socket(hostname: &str, port: u16) -> nix::Result<RawFd> {
    todo!()
}
...
mod ffi {
    ...
    /// Return `sockfd` if successful, `-1` on failure.
    #[no_mangle]
    pub extern "C" fn connect_socket(hostname: *const c_char, port: c_int) -> c_int {
        let c_str: &CStr = unsafe { CStr::from_ptr(hostname) };
        super::connect_socket(c_str.to_str().unwrap(), port.try_into().unwrap()).unwrap_or(-1)
    }
